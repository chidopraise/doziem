<div class="header text-center mt-5 pt-5">
	<h1>Wanted Persons</h1>
	<p>Help Us Apprehend Them</p>
</div>

<div class="container content">
	<div class="row">
		<?php
			$fetcher->wanted();
		?>
		<!--------------------
		<div class="col-md-4">
			<div class="missing-person-card">
				<img src="../images/wanted.jfif" alt="Wanted Person">
				<h2>John Doe</h2>
				<p style="color: #000;">Last seen: June 21, 2024</p>
				<p style="color: #000;">Location: New York, NY</p>
				<p style="color: #000;">Description: 6'0", brown hair, green eyes</p>
				<p class="reward">Reward: $1,000,000</p>
			</div>
		</div>
		<div class="col-md-4">
			<div class="missing-person-card">
				<img src="../images/wanted1.jfif" alt="Wanted Person">
				<h2>Jane Smith</h2>
				<p style="color: #000;">Last seen: June 20, 2024</p>
				<p style="color: #000;">Location: Los Angeles, CA</p>
				<p style="color: #000;">Description: 5'6", blonde hair, blue eyes</p>
				<p class="reward">Reward: $500,000</p>
			</div>
		</div>
		<div class="col-md-4">
			<div class="missing-person-card">
				<img src="../images/wanted2.jfif" alt="Wanted Person">
				<h2>Michael Johnson</h2>
				<p style="color: #000;">Last seen: June 19, 2024</p>
				<p style="color: #000;">Location: Chicago, IL</p>
				<p style="color: #000;">Description: 5'10", black hair, brown eyes</p>
				<p class="reward">Reward: $200,000</p>
			</div>
		</div>
		------>
	</div>
</div>